using System;
using Microsoft.EntityFrameworkCore.Migrations;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace webOdev.Migrations
{
    /// <inheritdoc />
    public partial class SeansSistemiGecis : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropForeignKey(
                name: "FK_Randevular_Antrenorler_AntrenorId",
                table: "Randevular");

            migrationBuilder.DropForeignKey(
                name: "FK_Randevular_Hizmetler_HizmetId",
                table: "Randevular");

            migrationBuilder.DropTable(
                name: "AntrenorMusaitlikleri");

            migrationBuilder.DropIndex(
                name: "IX_Randevular_HizmetId",
                table: "Randevular");

            migrationBuilder.DropColumn(
                name: "HizmetId",
                table: "Randevular");

            migrationBuilder.RenameColumn(
                name: "RandevuTarihi",
                table: "Randevular",
                newName: "IslemTarihi");

            migrationBuilder.RenameColumn(
                name: "AntrenorId",
                table: "Randevular",
                newName: "DersSeansId");

            migrationBuilder.RenameIndex(
                name: "IX_Randevular_AntrenorId",
                table: "Randevular",
                newName: "IX_Randevular_DersSeansId");

            migrationBuilder.AlterColumn<string>(
                name: "UzmanlikAlani",
                table: "Antrenorler",
                type: "text",
                nullable: false,
                oldClrType: typeof(string),
                oldType: "character varying(200)",
                oldMaxLength: 200);

            migrationBuilder.AlterColumn<string>(
                name: "AdSoyad",
                table: "Antrenorler",
                type: "text",
                nullable: false,
                oldClrType: typeof(string),
                oldType: "character varying(150)",
                oldMaxLength: 150);

            migrationBuilder.CreateTable(
                name: "DersSeanslari",
                columns: table => new
                {
                    Id = table.Column<int>(type: "integer", nullable: false)
                        .Annotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn),
                    AntrenorId = table.Column<int>(type: "integer", nullable: false),
                    TarihSaat = table.Column<DateTime>(type: "timestamp with time zone", nullable: false),
                    DoluMu = table.Column<bool>(type: "boolean", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_DersSeanslari", x => x.Id);
                    table.ForeignKey(
                        name: "FK_DersSeanslari_Antrenorler_AntrenorId",
                        column: x => x.AntrenorId,
                        principalTable: "Antrenorler",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_DersSeanslari_AntrenorId",
                table: "DersSeanslari",
                column: "AntrenorId");

            migrationBuilder.AddForeignKey(
                name: "FK_Randevular_DersSeanslari_DersSeansId",
                table: "Randevular",
                column: "DersSeansId",
                principalTable: "DersSeanslari",
                principalColumn: "Id",
                onDelete: ReferentialAction.Cascade);
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropForeignKey(
                name: "FK_Randevular_DersSeanslari_DersSeansId",
                table: "Randevular");

            migrationBuilder.DropTable(
                name: "DersSeanslari");

            migrationBuilder.RenameColumn(
                name: "IslemTarihi",
                table: "Randevular",
                newName: "RandevuTarihi");

            migrationBuilder.RenameColumn(
                name: "DersSeansId",
                table: "Randevular",
                newName: "AntrenorId");

            migrationBuilder.RenameIndex(
                name: "IX_Randevular_DersSeansId",
                table: "Randevular",
                newName: "IX_Randevular_AntrenorId");

            migrationBuilder.AddColumn<int>(
                name: "HizmetId",
                table: "Randevular",
                type: "integer",
                nullable: true);

            migrationBuilder.AlterColumn<string>(
                name: "UzmanlikAlani",
                table: "Antrenorler",
                type: "character varying(200)",
                maxLength: 200,
                nullable: false,
                oldClrType: typeof(string),
                oldType: "text");

            migrationBuilder.AlterColumn<string>(
                name: "AdSoyad",
                table: "Antrenorler",
                type: "character varying(150)",
                maxLength: 150,
                nullable: false,
                oldClrType: typeof(string),
                oldType: "text");

            migrationBuilder.CreateTable(
                name: "AntrenorMusaitlikleri",
                columns: table => new
                {
                    Id = table.Column<int>(type: "integer", nullable: false)
                        .Annotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn),
                    AntrenorId = table.Column<int>(type: "integer", nullable: false),
                    BaslangicSaati = table.Column<TimeOnly>(type: "time without time zone", nullable: false),
                    BitisSaati = table.Column<TimeOnly>(type: "time without time zone", nullable: false),
                    Gun = table.Column<int>(type: "integer", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_AntrenorMusaitlikleri", x => x.Id);
                    table.ForeignKey(
                        name: "FK_AntrenorMusaitlikleri_Antrenorler_AntrenorId",
                        column: x => x.AntrenorId,
                        principalTable: "Antrenorler",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_Randevular_HizmetId",
                table: "Randevular",
                column: "HizmetId");

            migrationBuilder.CreateIndex(
                name: "IX_AntrenorMusaitlikleri_AntrenorId",
                table: "AntrenorMusaitlikleri",
                column: "AntrenorId");

            migrationBuilder.AddForeignKey(
                name: "FK_Randevular_Antrenorler_AntrenorId",
                table: "Randevular",
                column: "AntrenorId",
                principalTable: "Antrenorler",
                principalColumn: "Id",
                onDelete: ReferentialAction.Cascade);

            migrationBuilder.AddForeignKey(
                name: "FK_Randevular_Hizmetler_HizmetId",
                table: "Randevular",
                column: "HizmetId",
                principalTable: "Hizmetler",
                principalColumn: "Id");
        }
    }
}
