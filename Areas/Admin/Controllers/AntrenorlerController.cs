...
namespace webOdev.Areas.Admin.Controllers
{
    ...
    public class AntrenorlerController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SeansSil(int seansId)
        {
            // 1. Önce silinecek seansı bul (Include kullanmıyoruz artık)
            var seans = await _context.DersSeanslari.FindAsync(seansId);

            if (seans != null)
            {
                int antrenorId = seans.AntrenorId; // Geri döneceğimiz sayfayı bilmek için

                // 2. Bu seansa ait randevuları MANUEL buluyoruz (Hata vermez)
                var bagliRandevular = _context.Randevular
                                        .Where(r => r.DersSeansId == seansId)
                                        .ToList();

                // 3. Varsa bu randevuların hepsini sil
                if (bagliRandevular.Any())
                {
                    _context.Randevular.RemoveRange(bagliRandevular);
                }

                // 4. Sonra dersin kendisini sil
                _context.DersSeanslari.Remove(seans);
                await _context.SaveChangesAsync();

                TempData["Basari"] = "Ders ve kayıtlı randevular başarıyla silindi.";

                // 5. Antrenörün sayfasına geri dön
                return RedirectToAction(nameof(Edit), new { id = antrenorId });
            }
